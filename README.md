...
Flask 기반 ONVIF IP Camera Web Monitoring Application.  
Network 내 ONVIF Camera를 자동으로 탐색하고, Browser에서 실시간 Streaming과 Server Side 녹화를 지원합니다.
...
- **외부 접속** — Cloudflare Tunnel (cloudflared) Hosting으로 Internet 어디서나 접속 가능
...
- Python 3.12+
- [go2rtc](https://github.com/AlexxIT/go2rtc/releases) 실행 File을 Project Folder에 배치
...
# go2rtc.yaml.example을 복사하여 go2rtc.yaml 생성 후 Stream 설정
cp go2rtc.yaml.example go2rtc.yaml

# go2rtc.exe를 Project Folder에 복사 후 실행
python bridge.py
```
